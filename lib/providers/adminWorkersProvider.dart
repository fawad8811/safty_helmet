  ...
  Future<void> fetchAllWorkers() async {
    try {
      ...
      final node1Worker = WorkersModel(
        ...
        workerLatitude:
            double.tryParse(node1.child('latitude').value.toString()) ?? 0.0,
        workerLongitude:
            double.tryParse(node1.child('longitude').value.toString()) ?? 0.0,

        // workerLatitude: _parseDouble(node1.child('latitude').value.toString()),
        // workerLongitude: _parseDouble(node1.child('longitude').value.toString()),
      );
      ...
      final node2Worker = WorkersModel(
        ...
        workerLatitude: _parseDouble(node2.child('latitude').value.toString()),
        workerLongitude:
            _parseDouble(node2.child('longitude').value.toString()),
      );
      ...
      database.ref('Zahid_Mehmood').onValue.listen((event) {
        ...
        workersList[0].workerLongitude = _parseDouble(longitude);
        workersList[0].workerLatitude = _parseDouble(latitude);
      ...
      database.ref('Syed_Ghafran_Haider').onValue.listen((event) {
        ...
        workersList[1].workerLongitude = _parseDouble(longitude);
        workersList[1].workerLatitude = _parseDouble(latitude);
